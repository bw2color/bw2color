...
from dataset.embedding import prepare_image_for_inception, maybe_download_inception, inception_resnet_v2, inception_resnet_v2_arg_scope
# from embedding.inception_resnet_v2 import inception_resnet_v2, inception_resnet_v2_arg_scope
# from embedding.inception_utils import prepare_image_for_inception, maybe_download_inception
...
class LabImagenetBatcher:
    def __init__(self, inputs_dir: str, records_dir: str, checkpoint_source: str):
        # 判断input_dir是不是一个目录
        ...
        self.records_names_gen = progressive_filename_generator(join(records_dir, 'lab_images_{}.tfrecord'))
...
if __name__ == '__main__':
    ...
    # 输入文件夹是一个包含299x299图片的文件夹
    # 将图片序列化至tfrecord文件，每batch的数量由默认参数指定
    ...
    LabImagenetBatcher(args.inputs, args.records, args.checkpoint).batch_all(args.batch_size)